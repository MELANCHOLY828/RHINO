...
def generate_mesh(model, N, return_sdf=False, num_outputs=4, model_name='model'):
    ...
    os.makedirs('/data/liufengyi/Results/sdf/logs1/bacon_armadillo/outputs/meshes',  exist_ok=True)
    mesh.export(f"/data/liufengyi/Results/sdf/logs1/bacon_armadillo/outputs/meshes/{model_name}_{idx+1}.obj")
...
def generate_mesh_adaptive(model, model_name):
    with torch.no_grad():
        ...
        mesh = trimesh.Trimesh(vertices=vertices, faces=triangles)
        # mesh.vertices = (mesh.vertices - b) / (a - b) * 0.9 - 0.45
        ...
        # # lucy
        # a = 25.523341576746198
        # b = 486.2661900195924
        # mesh.vertices = (mesh.vertices - a) / (b - a) * 0.9 - 0.45              
        # #thai
        ...
        mesh.vertices = (mesh.vertices / N - 0.5) + 0.5/N
        path = f'/data/liufengyi/Results/sdf/logs1/{model_name}/outputs/meshes1'
...
def generate_mesh_adaptive_(model, model_name):
    with torch.no_grad():
        ...
        # mesh.vertices = (mesh.vertices - mesh.vertices.min()) / (mesh.vertices.max() - mesh.vertices.min()) * 0.9 - 0.45
        mesh.vertices = (mesh.vertices / N - 0.5) + 0.5/N
        path = f'/data/liufengyi/Results/sdf/logs1/{model_name}/outputs/meshes'
        os.makedirs(path,  exist_ok=True)
        mesh.export(f"{path}/{model_name}_2.obj")
def export_meshes(adaptive=True):
    # bacon_ckpts = ['/data/liufengyi/MyCode/bacon/trained_models/dragon.pth',
    #                '/data/liufengyi/MyCode/bacon/trained_models/armadillo.pth',
    #                '/data/liufengyi/MyCode/bacon/trained_models/lucy.pth',
    #                '/data/liufengyi/MyCode/bacon/trained_models/thai.pth']
    ...
    # bacon_names = ['bacon_armadillo']
    bacon_names = ['bacon_dragon']
    # bacon_names = ['bacon_thai']
    # bacon_names = ['bacon_lucy']
    bacon_ckpts = [f"/data/liufengyi/Results/sdf/logs1/{bacon_names[0]}/checkpoints/model_final.pth"]
