...
def train():
    ...
    training.train_our(model=model, train_dataloader=dataloader, steps=opt.num_steps,
...
def init_model(opt):
    ...
    if opt.model_type == 'mfn':
        ...
        model = modules.CoordinateNet(nl='relu',
                                      ...
                                      use_sigmoid=False)
    elif opt.model_type == 'RHINO':  # mlp w relu + positional encoding
        model = modules.RHINO()
    elif opt.model_type == 'DINER':
        model = modules.DINER()
    else:
