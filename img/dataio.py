...
class ImageData(Dataset):
    ...
    def img_process(self,img):
        ...
        # [x, y] = torch.meshgrid(torch.linspace(0, W - 2, W-1), torch.linspace(0, H - 2, H-1))
        
        y = ((y.contiguous().view(-1, 1)) / (H-1) - 0.5) / 0.5
        x = ((x.contiguous().view(-1, 1)) / (W-1) - 0.5) / 0.5
        # y = ((y.contiguous().view(-1, 1)+0.5) / (H-1) - 0.5) / 0.5
        # x = ((x.contiguous().view(-1, 1)+0.5) / (W-1) - 0.5) / 0.5        
...
class ImageData_interp(Dataset):
    def __init__(self,
                image_path,
                sidelength,
                grayscale,
        ...
        self.image = self.normalize(self.image,sidelength)
        self.xy,self.xy1,self.xy2,self.xy3,\
            self.rgb,self.rgb1,self.rgb2,self.rgb3 = self.img_process(self.image)
        self.shape = self.image.shape
    ...
    def img_process(self,img):
        ...
        y = ((y.contiguous()) / (H-1) - 0.5) / 0.5
        x = ((x.contiguous()) / (W-1) - 0.5) / 0.5
        xy = torch.stack([x, y], dim=-1)
        xy1 = xy[::2,::2].reshape(-1,2)
        xy2 = xy[1::2,1::2].reshape(-1,2)
        xy3 = torch.cat([xy[1::2].reshape(-1,2),xy[::2,1::2].reshape(-1,2)],dim=0)
        rgb = img.reshape(-1,C)
        rgb1 = img[::2,::2].reshape(-1,C)
        rgb2 = img[1::2,1::2].reshape(-1,C)
        rgb3 = torch.cat([img[1::2].reshape(-1,C),img[::2,1::2].reshape(-1,C)],dim=0)
        ...
        return xy,xy1,xy2,xy3,rgb,rgb1,rgb2,rgb3

    def __len__(self):
        return self.rgb1.shape[0]

    def __getitem__(self, idx):
        return self.xy,self.xy1, self.xy2, self.xy3,self.rgb, self.rgb1, self.rgb2, self.rgb3
