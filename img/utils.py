...
def render_raw_image(model,save_path,img_resolution,gray = False,linear = False):
    ...
    if model.interp:
        ...
        x = ((x.contiguous().view(-1, 1)+0.5) / (W) - 0.5) / 0.5
        y = ((y.contiguous().view(-1, 1)+0.5) / (H) - 0.5) / 0.5 
    ...
    io.imsave(save_path,img)
@torch.no_grad()
def render_raw_image_interp(model,xy,save_path,img_resolution,gray = False,linear = False):
    device = torch.device('cuda')    
    H = int(math.sqrt(xy.shape[0]))
    W = int(math.sqrt(xy.shape[0]))
